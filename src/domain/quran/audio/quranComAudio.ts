...
const QURAN_COM_AYAH_AUDIO_BASE = "https://audio.qurancdn.com/";
...
function buildWordsByPosition(
  words: QuranComWord[] | undefined,
): Map<number, string> {
...
function parseChapterAyahSegments(
  ...
  for (let index = 0; index < raw.length; index += 1) {
    ...
    if (row.length === 2) {
      ...
      if (typeof position !== "number" || typeof absStart !== "number")
        continue;
...
export function mergeWordSegments(
  ...
  merged.sort((a, b) => a.startMs - b.startMs || a.position - b.position);
