...
describe("quranReaderRoutes", () => {
  it("builds numeric shareable paths", () => {
    expect(buildQuranReaderPath(42)).toBe("/quran/page/42");
    expect(buildQuranSurahPath(2)).toBe("/quran/surah/2");
  ...
  it("detects quran reader routes", () => {
    ...
    expect(isQuranReaderPath("/quran/page/5")).toBe(true);
    expect(isQuranReaderPath("/quran/surah/5")).toBe(true);
  ...
  it("detects layout from params", () => {
    ...
    expect(resolveLayoutMode({ pageNumber: "42" }, "/quran/page/42")).toBe(
      "page",
    );
  });
  ...
  it("resolves page and surah from numeric params", () => {
    ...
    expect(
      resolveReaderPage({ first: "42", second: "2" }, sampleVerses, 604),
    ).toBe(42);
    expect(
      resolveReaderPage(
  ...
  it("builds canonical paths", () => {
    expect(buildCanonicalReaderPath({ first: "2" }, sampleVerses, 604)).toBe(
      "/quran/surah/2",
    );
    expect(
  ...
  it("normalizes malformed and out-of-range canonical paths", () => {
    expect(normalizeCanonicalReaderPath("/quran/page/foo")).toBe(
      "/quran/page/1",
    );
    expect(normalizeCanonicalReaderPath("/quran/page/0")).toBe("/quran/page/1");
    expect(normalizeCanonicalReaderPath("/quran/page/999")).toBe(
      "/quran/page/604",
    );
    expect(normalizeCanonicalReaderPath("/quran/surah/0002")).toBe(
      "/quran/surah/2",
    );
    expect(normalizeCanonicalReaderPath("/quran/surah/nope/ayah/7")).toBe(
      "/quran/surah/1/ayah/7",
    );
    expect(normalizeCanonicalReaderPath("/quran/surah/2/ayah/255")).toBeNull();
  });

  it("redirects legacy verbose paths", () => {
    expect(legacyQuranPathRedirect("/quran/page/42/surah/2")).toBe(
      "/quran/page/42",
    );
    expect(legacyQuranPathRedirect("/quran/surah/2/ayah/255")).toBe(null);
    expect(legacyQuranPathRedirect("/quran/surah/2")).toBe(null);
    expect(legacyQuranPathRedirect("/quran/page/42")).toBe(null);
    expect(legacyQuranPathRedirect("/quran/scroll/42")).toBe("/quran/page/42");
    expect(legacyQuranPathRedirect("/quran/42/2")).toBe("/quran/page/42");
    expect(legacyQuranPathRedirect("/quran/2/255")).toBe(
      "/quran/surah/2/ayah/255",
    );
    expect(legacyQuranPathRedirect("/quran/2")).toBe("/quran/surah/2");
  });
});
